...
Read `spec/skills/adr_tracking.md`.
...
- reconcile report markers (see `report_tracking.md`): run
  `grep -rn "\[open\]|\[needs-decision\]" spec/report/` and mark `[acted]`
  any item this work resolved, appending a one-line note; the reported set
  must contain only items that are genuinely still open or undecided
...
the intended design. At the commit/finish point of any task you MUST:
check whether the work made any document stale or inaccurate, update any
stale document in the same change so the docs never lag behind the
implementation, and ask the owner in plain text when a mismatch is
ambiguous (for example, when it is unclear whether the code or the
document reflects the intended behavior). Do not guess.