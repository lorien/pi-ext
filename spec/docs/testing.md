...
no reminder before the mode is ever applied, the ON reminder returned on
every prompt while applied, the OFF reminder returned on every prompt
after the lift, and the context filter keeping only the newest mode
message.
...
It also asserts that all four shipped prompt files load with content. The
